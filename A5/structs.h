...
struct Planet {
	...
	Planet(float radius, float distance, float localPeriod, float orbitalPeriod, float axialTilt, MyTexture *texture)
	{
		...
		if (localPeriod > 0)
			this->localRotPerSec = (float)(2 * M_PI) /(localPeriod*3600.0f);
		else
			this->localRotPerSec = 0;
		
		if (orbitalPeriod)
			this->orbitalRotPerSec = (float)(2 * M_PI) /(orbitalPeriod*3600.0f);
		else
		...
		this->axialTiltMatrix = glm::rotate(glm::mat4(), (float)(axialTilt * M_PI) / 180.0f, glm::vec3(0,0,1));
	}
